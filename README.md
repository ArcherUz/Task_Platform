...
1. 将参数中为空的文件、字节、sign剔除
2. 排序，对参数中所有的key进行从小到大排序
3. 将排序后的参数与其对应值，组合成“参数=参数值”的格式，并且把这些参数用&字符连接起来，此时生成的字符串为代签名字符串。
4. 如有字典，也应转换为字符串。字符串中间不能有空格。
...
5. 调用签名函数进行签名，并进行Base64编码。

​	把签名再添加到字典中。params[sign] = 签名
...
- url，查看所有的订单（读取个人的交易记录的表 category=2)
- 去支付（把自己的ID传到后台）
  - 根据ID获取：价格、订单号
  - 生成支付宝跳转链接
  - 跳转到支付宝去支付
